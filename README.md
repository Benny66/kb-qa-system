...
- 支持多轮上下文，能够继续追问
- 支持在同一知识库下创建多个独立会话
- **流式输出**：打字机效果，实时逐字展示回答，减少等待焦虑
- **中止生成**：生成过程中可随时点击停止按钮，中止后保留已生成内容
...
管理员账号密码在 `kb-qa-backend/.env` 中配置，登录页不展示账号密码：

| 配置项 | 说明 |
|---|---|
| `PRESET_ADMIN_USERNAME` | 用户名，默认 `admin` |
| `PRESET_ADMIN_PASSWORD` | 密码，必填 |

首次启动时若账号不存在会按上述配置创建；若账号已存在，则用配置中的密码覆盖（改配置后重启即生效）。
...
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/api/chat` | 发起问答（非流式），支持 `session_id` 持续会话 |
| POST | `/api/chat/stream` | **SSE 流式问答**，逐字推送模型输出，支持中止 |
